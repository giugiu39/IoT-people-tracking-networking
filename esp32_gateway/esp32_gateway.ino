...
void analyzeMqttPublish(
    ...
    size_t fixedHeaderSize =
        1 + remainingLengthSize;

    size_t variableHeaderSize =
        2 + topicLength;
    ...
    Serial.printf(
        "Transport         : MQTT over TLS over TCP\n"
    );
    ...
    Serial.printf(
        "%02X ",
        (uint8_t)(topicLength >> 8)
    );

    // Topic Length - LSB
    Serial.printf(
        "%02X ",
        (uint8_t)(topicLength & 0xFF)
    );
    ...
    void onWrite(BLECharacteristic *pCharacteristic) {
        ...
        if (rxValue.length() > 0) {
            ...
            if (success) {
                ...
                Serial.println(
                    "[MQTT] Publish accepted by client."
                );
...
class MyServerCallbacks : public BLEServerCallbacks {
    ...
    void onDisconnect(
        ...
        Serial.println(
            "[BLE] Client disconnesso."
        );

        Serial.println(
            "[BLE] Riavvio advertising..."
        );
