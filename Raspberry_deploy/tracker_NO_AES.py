            ...
            try:
                ...
                async with BleakClient(device) as client:
                    while client.is_connected:
                        ...
                        print(f"   [BLE Success] Evento {event_data['event_id']} spedito via BLE (IN CHIARO)")
...
def draw_laser_bbox(frame, x1, y1, x2, y2, color=(0, 255, 255), thickness=1, length=15):
    ...
    cv2.line(frame, (x2, y1), (x2, y2 + length), color, thickness, l_type)
...
# EVENT GENERATOR (NO_AES log path)
class EventGenerator:
   
    def __init__(self, log_path="edge_ai/output/events_NO_AES.jsonl"):
        self.person_zones = {}   
        self.events = []
...
def run_tracker(video_source=0, show=True):
    ...
    print("[tracker] Running (NO_AES)... Press Q to quit.")
